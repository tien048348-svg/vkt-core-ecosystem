...
export const PODCAST_TOPICS = [
  { id: 'psychology', label: '🧠 Tâm Lý Học Tội Phạm & Hành Vi (Psychology)' },
  { id: 'startup', label: '🚀 Khởi Nghiệp Thực Chiến (Startup & Hustle)' },
  { id: 'true_crime', label: '🕵️ True Crime & Hồ Sơ Vụ Án (True Crime)' },
  { id: 'deep_healing', label: '🫂 Chữa Lành Tổn Thương (Deep Healing)' },
  { id: 'tech_future', label: '🤖 Tương Lai Công Nghệ (Tech & Future)' },
  { id: 'philosophy', label: '📚 Triết Lý Sống & Nhân Sinh (Philosophy)' },
  { id: 'relationship', label: '❤️ Tình Yêu & Mối Quan Hệ (Relationships)' },
  { id: 'finance', label: '💰 Tài Chính Cá Nhân (Personal Finance)' },
  { id: 'mystery', label: '👽 Chuyện Bí Ẩn Tâm Linh (Mystery)' }
];

const MICRO_CONTEXTS: Record<string, string[]> = {
  'psychology': [
    'Phân tích hành vi tội phạm thao túng tâm lý', 'Bóc tách hội chứng kẻ mạo danh ở người trẻ',
    'Giải mã ngôn ngữ cơ thể trong cuộc đàm phán', 'Hiệu ứng đám đông trong xã hội hiện đại'
  ],
  'startup': [
    'Bài học xương máu từ startup gọi vốn thất bại', 'Kỹ năng sinh tồn khi công ty cạn tiền',
    'Bí quyết chốt sale của sói già phố Wall', 'Câu chuyện Founder bị hất cẳng khỏi công ty'
  ],
  'true_crime': [
    'Hồ sơ vụ án bí ẩn chưa có lời giải đáp', 'Lời khai lạnh người của kẻ sát nhân hàng loạt',
    'Phân tích pháp y rùng rợn hiện trường', 'Cuộc truy lùng gắt gao giữa đêm đen'
  ],
  'deep_healing': [
    'Xoa dịu đứa trẻ bên trong đầy vết xước', 'Tha thứ cho sự độc hại của gia đình',
    'Bước qua trầm cảm tuổi trưởng thành', 'Học cách yêu thương bản thân từ vực thẳm'
  ],
  'tech_future': [
    'Mặt trái đáng sợ của Trí tuệ nhân tạo (AI)', 'Cuộc đua chip bán dẫn toàn cầu',
    'Thế giới sẽ ra sao khi robot có cảm xúc', 'Góc khuất của các gã khổng lồ thung lũng Silicon'
  ],
  'philosophy': [
    'Chủ nghĩa khắc kỷ giữa thời đại hỗn loạn', 'Ý nghĩa cuộc sống trong mắt triết gia',
    'Nghệ thuật buông bỏ sự hoàn hảo', 'Hạnh phúc đích thực không đến từ vật chất'
  ],
  'relationship': [
    'Red flags trong tình yêu độc hại cần né gấp', 'Gắn kết bền chặt sau hôn nhân rạn nứt',
    'Nghệ thuật giao tiếp không bạo lực', 'Khi người thứ 3 xuất hiện trong mối quan hệ'
  ],
  'finance': [
    'Cạm bẫy của thẻ tín dụng và vay tiêu dùng', 'Bí mật tự do tài chính tuổi 30',
    'Đầu tư tiền ảo: Cờ bạc hay xu hướng tương lai', 'Sai lầm tài chính khiến bạn mãi nghèo'
  ],
  'mystery': [
    'Giải mã những hiện tượng siêu nhiên dị thường', 'Những câu chuyện có thật về thế giới bên kia',
    'Thuyết âm mưu về người ngoài hành tinh', 'Bí ẩn chưa lời giải của vũ trụ bao la'
  ]
};
...
const ScriptModule: React.FC<Props> = ({ onScriptGenerated, onAudioRefined, initialTopic = '', uiLang }) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const topicObj = PODCAST_TOPICS.find(t => t.id === podcastTopic);
      const contexts = MICRO_CONTEXTS[podcastTopic] || ['Trò chuyện sâu sắc'];
...
5. GENERATE JSON OBJECT.`;
      ...
      const finalJson = {
        mode_detected: targetDuration < 3 ? "Quick Insight" : targetDuration <= 10 ? "Story Weaver" : "Deep Analysis",
...
- Maintain the alternating characters as specified in the original input.`;
      ...
      if (res?.refined_scenes) {
        ...
        try {
          const currentState = { segments: newSegments, topic: topic, scriptData: scriptData };
          localStorage.setItem('podcast_autosave_script', JSON.stringify(currentState));
        } catch (e) { console.error('Failed to autosave refined audio'); }
