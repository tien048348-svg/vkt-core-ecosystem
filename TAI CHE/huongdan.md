...
## 🚀 CHANGELOG: V16.5.2 VOICE PROFILE AGE & ACCENT RESTORATION (Tái Chế - 2026-05-19)

- **Expanded Voice Profile Prompts (`src/data/prompts.ts`):** Bổ sung hoàn chỉnh các trường `"gender"`, `"age"`, `"accent"` vào định nghĩa cấu trúc JSON của thẻ `voice_profile` ở cả hai Prompts chính (**Script Writer** và **Audio Re-engineering**), ép AI sinh ra chính xác chi tiết độ tuổi nhân vật (như Tấm 18 tuổi, Cám 17 tuổi...) và giọng đọc vùng miền chuẩn.
- **5-Column Voice Profile UI (`ScriptModule.tsx` & `StudioModule.tsx`):** Thiết kế lại giao diện hiển thị từ 3 cột lên 5 cột cân đối, hiển thị trực quan dải thông số: `AGE & DETAILS` | `ACCENT` | `TIMBRE` | `TONE` | `SPEED` đáp ứng chuẩn vận hành tối ưu.
- **Production Live Deploy Sync:** Deploy bản hoàn thiện lên Vercel chính thức `taiche.kiemtienvu.com`.

## 🚀 CHANGELOG: V16.5.1 FINAL PERFECT RESTORATION & 16-STYLE SYNC (Tái Chế - 2026-05-19)

- **SpyModule.tsx Placeholder Configuration:** Thay thế toàn bộ các placeholders `[TEN_NGACH_EN]` và `[TEN_NGACH_VI]` thành `Recycled Art & Vietnamese Folklore` và `Truyện Cổ Tích Tái Chế` để hiển thị hoàn hảo nhất.
- **index.html Meta Configuration:** Thay thế `[TEN_NGACH]` và `[MO_TA_NGACH]` thành `RECYCLE STYLES` và `Truyện cổ tích Việt Nam bằng vật liệu tái chế.`
- **Complete 16-Style Restoration (constants.ts):** Khôi phục và bổ sung đầy đủ hệ thống 16 phong cách visual tái chế (driftwood_twig, metal_can_origami, hybrid_metal_clay, hybrid_folk_automata, v.v.) với các prompt ép buộc hình ảnh (prompt_enforce) siêu chi tiết, độc lạ, đậm chất điện ảnh thủ công.
- **AI Proposal Synced (ScriptModule.tsx):** Cập nhật hệ thống đề xuất phong cách thông minh của AI (`STYLE_RECOMMENDATION_PROMPT`) để nạp đủ 16 phong cách, cho phép AI tự động phân tích và đưa ra các đề xuất phong cách độc bản cho người dùng.
- **Header V50.0 Cleanup:** Xóa bỏ hoàn toàn nhãn phiên bản `V50.0` lỗi thời dưới logo tiêu đề để giữ giao diện sạch đẹp theo đúng yêu cầu.
- **Duration Real-Time Clamp Safeguard:** Thiết lập bộ gạt tự động thời lượng tối đa về 10 phút ngay khi người dùng gõ hoặc bấm tăng vượt mức 10, hiển thị cảnh báo Toast màu vàng trực quan, ngăn chặn triệt để tình trạng vượt quá giới hạn 10 phút.
- **Vercel Production Aliasing:** Deploy thành công phiên bản hoàn mỹ không lỗi lên tên miền chính thức `taiche.kiemtienvu.com`.
...
|------|---------|----------------|
| `recylestyles.txt` | 📦 Kho phong cách visual | 16 phong cách nghệ thuật tái chế chi tiết (Stop-Motion, Đông Hồ, Múa Rối Nước, Plastic Mosaic, Fabric Collage, Pop-up Cardboard, Lá Khô & Hạt, Metal Can, Driftwood, v.v.) |
...
├── eslint.config.js
├── .gitignore
├── recylestyles.txt        ← 📦 KHO PHONG CÁCH VISUAL (16 styles)
...
│   ├── 📁 data/
│   │   ├── constants.ts    ← ⭐ TARGET_MARKETS, VISUAL_STYLES (17 styles), TAB_COLORS
...
### 17 Visual Styles có sẵn (constants.ts):

#### Phong cách đơn (7 gốc + 6 mở rộng = 13 styles):
...
| **v4.0** | **2026-05-19** | **Hợp nhất VKT MASTER AGENT spec: +7 tính năng mới (Audio Truncation Shield, Dynamic Speed Matrix, Compound Word Lock, Breath Control, Global English Mode, VEO3 AUTO-SHIELD, Expanded Voice Profile). Mở rộng 8→17 visual styles. Nâng cấp code failsafe.** |
| v4.1 | 2026-05-19 | Nâng cấp kiểm soát thời lượng, tự động clamp gạt thời lượng tối đa về 10 phút và cảnh báo toast tiếng Anh/Việt. |
| v4.2 | 2026-05-19 | Nâng cấp lưới Voice Profile thành 5 cột hoàn mỹ ngang hàng (AGE & DETAILS, ACCENT, TIMBRE, TONE, SPEED). |
| v4.3 | 2026-05-19 | Xuất Excel CSV phân dòng trống cách biệt giữa các cảnh giúp nâng cao độ trực quan khi làm việc trên Excel. |
| **v4.4** | **2026-05-19** | **Tích hợp tùy chọn xuất Excel Kịch Bản V2 (CSV2) gộp nhãn trực tiếp vào đầu mỗi ô (không dòng tiêu đề, gộp Time & Section, chèn dòng trống phân cách) đồng bộ cả 3 dự án.** |
| **v4.5** | **2026-05-20** | **Khóa cứng nhân vật Mẹ & Bé là NGƯỜI (Human) cho phong cách Học Tiếng Anh (Mẹ & Bé), loại bỏ triệt để hiện tượng AI tự sinh động vật (gấu, thỏ) làm đứt gãy tính đồng nhất của video mẫu trẻ em.** |
| **v4.6** | **2026-05-20** | **Khóa cứng đạo cụ học tập chân thực (Realistic Props) cho phong cách Học Tiếng Anh (Mẹ & Bé). Loại bỏ hoàn toàn mô tả đất sét/vải nỉ đối với thẻ học, trái cây, đồ chơi để đồng nhất chuẩn Pixar 3D hiện đại như video mẫu.** |
| **v4.7** | **2026-05-21** | **Cập nhật Bức tường lửa bản quyền (Copyright Firewall). Chặn nghiêm ngặt việc nhắc đến tên người thật, người nổi tiếng hoặc nhân vật có bản quyền trong video_prompt và image_prompt để khắc phục lỗi vi phạm chính sách của AI (DALL-E 3/Imagen/Veo3).** |
