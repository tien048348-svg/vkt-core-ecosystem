...
const StudioModule: React.FC<Props> = ({ segments, topic = '' }) => {
  ...
  const exportJSON_PRO = () => {
    ...
    for (let i = 0; i < segments.length; i += chunkSize) {
      const chunkSegs = segments.slice(i, i + chunkSize);
      const prevMemory = i > 0 && segments[i-1].voice_text ? "Tiếp nối: " + segments[i-1].voice_text.substring(0, 100) + "..." : "Khởi đầu câu chuyện.";
      chunks.push({
        chunk_id: `CHUNK_${Math.floor(i/chunkSize) + 1}`,
        scene_count: chunkSegs.length,
    ...
    const content = JSON.stringify(proData, null, 2);
    downloadFile(content, makeFileName(topic || '', 'project_PRO', 'json'), 'application/json;charset=utf-8');
    setShowExport(false);
    showToast('🚀 Tải Dự Án PRO (Smart Chunking) thành công!', 'success');
  };
  ...
  const exportCSV_PRO = () => {
    ...
    segments.forEach((s, i) => {
      ...
      const vp = s.voice_profile || {};
      const sfx = s.sfx_music_suggestion || s.sfx_suggestion || '';
      csv += `"${chunkId}","${prevMemory}",${i + 1},"${s.time}","${s.section}","${s.character}","${(s.voice_text || '').replace(/"/g, '""')}","${vp.speaker || ''}","${vp.gender || ''}","${vp.age || ''}","${vp.accent || ''}","${vp.timbre || ''}","${vp.tone || ''}","${vp.pacing || ''}","${vp.pacing_speed || ''}","${s.word_count || ''}","${s.audio_end_time || ''}","${vp.state || ''}","${sfx.replace(/"/g, '""')}","${(s.video_prompt || '').replace(/"/g, '""')}","${(s.image_prompt || '').replace(/"/g, '""')}"\n`;
    });
    downloadFile(csv, makeFileName(topic || '', 'kich_ban_PRO', 'csv'), 'text/csv;charset=utf-8;');
    setShowExport(false);
