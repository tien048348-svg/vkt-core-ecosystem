...
const translations = {
  vi: {
    script: {
      ...
      speakerMode: "Tùy chọn Giọng đọc",
      singleSpeaker: "Có Giọng Đọc (Lão Sư)",
      pureAsmr: "Không Giọng Đọc (ASMR)",
      singleSpeakerDesc: "Có giọng đọc, mặc định sử dụng 1 giọng dẫn truyện của Vị Lão Sư 70 Tuổi xuyên suốt.",
      pureAsmrDesc: "Hoàn toàn tĩnh lặng, không lời thoại. Tập trung 100% vào hình ảnh và nhạc nền (BGM/ASMR)."
    },
    common: {
  ...
  en: {
    script: {
      ...
      speakerMode: "Voice Options",
      singleSpeaker: "With Voice (Master)",
      pureAsmr: "No Voice (ASMR)",
      singleSpeakerDesc: "Features a single narrator voice of a 70-year-old Wise Master throughout the video.",
      pureAsmrDesc: "Absolutely no voiceover. Focus 100% on visuals and therapeutic Zen ASMR soundscapes."
    },
    common: {
...
const ScriptModule: React.FC<Props> = ({ segments, setSegments, scriptData, setScriptData, onScriptGenerated, onAudioRefined, initialTopic = '', uiLang, onNavigateToStudio, isAdmin, globalSettings }) => {
  ...
  const [speakerMode, setSpeakerMode] = useState<'single' | 'asmr'>('asmr');
    ...
    <div className="max-w-5xl mx-auto space-y-6 animate-[slideIn_0.4s_ease-out]">
      <div className="bg-[#12161e] border border-slate-700/30 p-6 rounded-2xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.5)]">
        ...
        <div className="space-y-4">
          ...
          {globalSettings.enableAudioRefinement !== false && (
            refiningAudio ? (
              <ProgressBar 
              ...
              <button onClick={handleAudioRefining} disabled={segments.length === 0 || loading || refiningAudio}
