...
const StudioModule: React.FC<Props> = ({ segments, topic = '' }) => {
  ...
  const exportJSON_PRO = () => {
    ...
    for (let i = 0; i < segments.length; i += chunkSize) {
      ...
      const slimSegs = chunkSegs.map((s: any, idx: number) => ({
        scene: i + idx + 1,
  ...
  const exportCSV_PRO = () => {
    if (!segments.length) return;
    let csv = '\uFEFFChunk_ID,Scene,Video Prompt,Image Prompt\n';
    const chunkSize = 25;
    segments.forEach((s, i) => {
      const chunkIdx = Math.floor(i / chunkSize) + 1;
      const chunkId = `CHUNK_${chunkIdx}`;
      csv += `"${chunkId}",${i + 1},"${(s.video_prompt || '').replace(/"/g, '""')}","${(s.image_prompt || '').replace(/"/g, '""')}"\n`;
    });
