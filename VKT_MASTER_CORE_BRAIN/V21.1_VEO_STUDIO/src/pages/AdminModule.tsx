...
interface AdminModuleProps {
  ...
  const [customMinSpeed, setCustomMinSpeed] = useState('');
  const [customMaxSpeed, setCustomMaxSpeed] = useState('');

  useEffect(() => {
    const val = localStorage.getItem('dharma_enable_pro_export');
    setProExportEnabled(val === 'true');
    setCustomMinSpeed(localStorage.getItem('dharma_admin_min_speed') || '');
    setCustomMaxSpeed(localStorage.getItem('dharma_admin_max_speed') || '');
  }, []);
        ...
        <div className="bg-amber-900/10 border border-amber-500/30 p-6 rounded-xl relative overflow-hidden">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-[#0a0e14]/50 border border-slate-700/50 p-4 rounded-lg flex items-center justify-between">
              <div>
                <div className="font-bold text-slate-200 flex items-center gap-2">
                  <i className="fa-solid fa-gauge-high text-pink-500"></i> Tùy chỉnh tốc độ AI (Admin Only)
                </div>
                <div className="text-xs text-slate-500 mt-2">Bật công tắc này để hiển thị bảng ép tốc độ ở trang chủ (chỉ dành cho Admin).</div>
              </div>
              <button 
                onClick={async () => {
                  const newSettings = { ...globalSettings, enableAdminSpeedOverride: !globalSettings.enableAdminSpeedOverride };
                  await setDoc(doc(db, 'dharma_settings', 'global_config'), newSettings);
                  showToast(newSettings.enableAdminSpeedOverride ? 'Đã BẬT Tùy chỉnh tốc độ AI!' : 'Đã TẮT Tùy chỉnh tốc độ AI!', 'success');
                }}
                className={`relative inline-flex h-6 w-11 shrink-0 items-center rounded-full transition-colors focus:outline-none ${
                  globalSettings.enableAdminSpeedOverride ? 'bg-amber-500' : 'bg-slate-600'
                }`}
              >
                <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                  globalSettings.enableAdminSpeedOverride ? 'translate-x-6' : 'translate-x-1'
                }`} />
              </button>
            </div>
