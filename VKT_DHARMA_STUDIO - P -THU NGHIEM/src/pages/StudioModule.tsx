...
const StudioModule: React.FC<Props> = ({ segments, topic = '' }) => {
  ...
  const exportJSON_PRO = () => {
    ...
    downloadFile(content, makeFileName(topic || '', 'render_PRO', 'json'), 'application/json;charset=utf-8');
    setShowExport(false);
    showToast('🚀 Tải JSON PRO (Render Only) thành công!', 'success');
  };
  ...
  const exportCSV_PRO = () => {
    ...
    segments.forEach((s, i) => {
      csv += `${i + 1},"${(s.video_prompt || '').replace(/"/g, '""')}","${(s.image_prompt || '').replace(/"/g, '""')}"\n`;
    });
    downloadFile(csv, makeFileName(topic || '', 'render_PRO', 'csv'), 'text/csv;charset=utf-8;');
    setShowExport(false);
    showToast('🚀 Tải CSV PRO (Render Only) thành công!', 'success');
  };
