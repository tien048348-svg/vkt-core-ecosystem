...
## 🎙️ TÍNH NĂNG: TINH CHỈNH THANH ÂM (Audio Re-Engineering V16.0)

> 💡 Tính năng hậu xử lý thanh âm — áp dụng **SAU KHI** kịch bản đã được tạo xong.
> Đã tích hợp sẵn trong `App.tsx` với prompt `SYSTEM_PROMPT_AUDIO_REENGINEERING`.

### Workflow

```
1. Tạo kịch bản tại Tab 2 "THE DOSSIER" → Bấm "KÍCH HOẠT BIÊN KỊCH MASTERMIND"
2. Kịch bản xuất hiện (scenes/chapters) → Bấm nút 🎙️ "TINH CHỈNH THANH ÂM (V16.0)"
3. AI tự động:
   ✅ GIỮ NGUYÊN 100% hình ảnh/prompt/director_note
   ✅ Tinh chỉnh voice_text (lời thoại)
   ✅ Thêm voice_profile (hồ sơ thanh âm)
   ✅ Cập nhật video_gen_voiceover (audio cue)
4. Kết quả hiển thị trong Tab 3 "EVIDENCE ROOM" với Voice Profile card
```

### 3 Quy Tắc Thép (Thiết Quân Luật)

| # | Quy tắc | Chi tiết |
|---|---|---|
| 1 | **Độc Tôn (Single Voice)** | Mỗi scene CHỈ 1 người nói. Cấm chồng lấn hội thoại |
| 2 | **Hồ Sơ Thanh Âm (Voice Profile)** | Mỗi scene phải có: Speaker, Timbre, Tone, Pacing, State (ON/OFF-SCREEN) |
| 3 | **Lời Thoại Nội Lực** | Dưới 40 từ, súc tích, đậm bản sắc nhân vật |

### Voice Profile hiển thị trong Studio

```
🎙️ [Tên người nói] ─── [ON-SCREEN / OFF-SCREEN]
┌─────────────┬─────────────┬─────────────┐
│ Chất giọng  │ Giọng điệu  │ Nhịp điệu   │
│ (Timbre)    │ (Tone)      │ (Pacing)    │
└─────────────┴─────────────┴─────────────┘
```

### Lưu ý khi clone ngách mới

- Prompt `SYSTEM_PROMPT_AUDIO_REENGINEERING` nằm trong `App.tsx`, **KHÔNG cần sửa** khi clone ngách mới
- Prompt tự thích ứng theo ngữ cảnh kịch bản (Crime, Philosophy, Horror, v.v.)
- Nút "TINH CHỈNH THANH ÂM" chỉ xuất hiện **sau khi** đã có kịch bản
- Có thể bấm nhiều lần để tinh chỉnh lại (counter hiển thị số lần đã refine)
