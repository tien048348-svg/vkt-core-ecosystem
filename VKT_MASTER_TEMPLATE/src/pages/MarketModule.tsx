    ...
    <div className="max-w-5xl mx-auto space-y-6 animate-[slideIn_0.4s_ease-out]">
      <div className="bg-[#12161e] border border-slate-700/30 p-6 rounded-2xl shadow-lg">
        ...
        <div className="flex gap-2 md:gap-4 mb-6 flex-col sm:flex-row">
          <input value={topic} onChange={e => setTopic(e.target.value)} className="flex-1 bg-[#0a0e14] border border-slate-700/50 rounded-lg p-3 text-sm text-white outline-none focus:border-yellow-500/50 placeholder-slate-600" placeholder="Nhập chủ đề (VD: Coaching, Khóa học quản trị cảm xúc)..." />
        ...
        {!result ? (
          <div className="h-64 flex flex-col items-center justify-center text-slate-500 border border-slate-700/30 border-dashed rounded-xl bg-slate-800/20">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-gradient-to-br from-pink-900/10 to-purple-900/10 border border-pink-500/20 rounded-xl p-5">
                ...
                {persona.psychographics?.pain_points && (
                  <div className="mt-4 p-3 bg-[#0a0e14]/40 rounded border border-slate-700/30">
              ...
              <div className="bg-gradient-to-br from-blue-900/10 to-cyan-900/10 border border-blue-500/20 rounded-xl p-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-3 bg-[#0a0e14]/40 rounded border border-slate-700/30"><div className="text-[10px] text-slate-500 uppercase mb-1">Quy mô</div><div className="text-sm text-white font-black font-mono">{potential.market_size || 'N/A'}</div></div>
                  <div className="p-3 bg-[#0a0e14]/40 rounded border border-slate-700/30"><div className="text-[10px] text-slate-500 uppercase mb-1">Tăng trưởng</div><div className="text-sm text-green-400 font-black font-mono">{potential.growth_rate || 'N/A'}</div></div>
                </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {products.map((cat: any, ci: number) => (
                    <div key={ci} className="bg-[#12161e] border border-slate-700/30 rounded-xl overflow-hidden hover:border-green-500/20 transition-colors">
                      <div className="p-3 bg-slate-800/20 border-b border-slate-700/30"><div className="text-[10px] text-green-400 font-bold uppercase">{cat.category}</div></div>
                        ...
                        {Array.isArray(cat.sourcing_links) && (
                          <div className="pt-2 border-t border-slate-700/30 space-y-1">
                            ...
                            {cat.sourcing_links.map((link: any, li: number) => (
                              <a key={li} href={link.url} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between p-1.5 rounded bg-[#0a0e14]/40 hover:bg-green-900/20 border border-transparent hover:border-green-500/30 transition-all">
            ...
            {/* Strategy + Profit */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-[#10141c] border border-slate-700/30 rounded-xl p-5">
              ...
              <div className="bg-gradient-to-br from-green-900/10 to-amber-900/10 border border-green-500/20 rounded-xl p-5">
                ...
                <div className="space-y-3">
                  {Object.values(calculator).map((scen: any, i: number) => (
                    <div key={i} className="bg-[#0a0e14]/50 p-3 rounded-lg border border-slate-700/30 flex justify-between items-center">
