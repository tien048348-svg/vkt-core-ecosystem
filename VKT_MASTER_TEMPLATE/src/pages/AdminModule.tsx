...
const AdminModule: React.FC<AdminModuleProps> = ({ uiLang, globalSettings }) => {
  ...
  const handleDownloadRollback = (version: string) => {
    const projectName = 'Dharma_Thu_Nghiem';
    const link = document.createElement('a');
    link.href = `/rollbacks/${version}_Rollback.zip`;
    link.download = `${projectName}_${version}_Rollback.zip`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    showToast(`Đã tải về bản sao lưu: ${projectName}_${version}_Rollback.zip`, 'success');
  };

  const handleUploadRollback = () => {
    ...
    input.onchange = (e: any) => {
      ...
      if (file) {
        alert(`THÔNG BÁO TỪ HỆ THỐNG:\n\nBạn đã chọn file: ${file.name}\n\nDo đây là hệ thống Cloud (Vercel), việc tải file lên giao diện web sẽ KHÔNG làm thay đổi mã nguồn ngay lập tức.\n\nĐỂ KHÔI PHỤC (ROLLBACK): Vui lòng mở khung chat với trợ lý Antigravity AI, kéo thả file ZIP này vào và ra lệnh:\n"Hãy khôi phục dự án bằng file ZIP này và Deploy lại."`);
        showToast('Đã nhận lệnh Khôi phục! Hãy gửi file cho AI.', 'warning');
      }
    };
    input.click();
  };
      ...
      <div className="flex-1 overflow-y-auto space-y-6">
        ...
        <div className="bg-[#12161e] border border-slate-700/30 p-6 rounded-xl relative overflow-hidden">
          ...
          <div className="bg-[#0a0e14]/50 border border-slate-700/50 p-4 rounded-lg flex items-center justify-between">
            <div>
              ...
              <div className="text-xs text-slate-500 mt-1">Sẽ hiển thị thêm các nút tải JSON PRO, CSV PRO (Chỉ trích xuất Scene, Video Prompt, Image Prompt để đưa vào máy Render).</div>
            </div>
        ...
        <div className="bg-[#12161e] border border-slate-700/30 p-6 rounded-xl relative overflow-hidden">
          ...
          <div className="flex items-start justify-between mb-6 gap-4">
            <p className="text-sm text-slate-400">Nơi quản lý mã nguồn (Rollback). Hãy tải về bản sao dự phòng hoặc tải lên mã nguồn cũ để yêu cầu AI phục hồi khi hệ thống gặp lỗi.</p>
            <button 
              onClick={handleUploadRollback}
              className="px-4 py-2 bg-amber-500 hover:bg-amber-400 text-black rounded text-xs font-bold transition-colors flex items-center gap-2 shadow-[0_0_15px_rgba(245,158,11,0.3)] shrink-0"
            >
              <i className="fa-solid fa-cloud-arrow-up text-sm"></i> Tải Mã Nguồn Lên (Restore)
            </button>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="bg-[#0a0e14]/80 border border-teal-900/50 p-4 rounded-lg flex items-center justify-between">
              <div>
                <div className="font-black text-teal-400 text-sm flex items-center gap-2">
                  V20.0 (All-In-One & VEO3 Shield) <span className="px-1.5 py-0.5 rounded bg-teal-900 text-[9px] text-white">HIỆN TẠI</span>
                </div>
                <div className="text-xs text-slate-500 mt-1">Đồng bộ hoàn hảo chuẩn V20. Cấu trúc All-in-one Video Prompt và Ma trận Tốc độ đọc.</div>
              </div>
            ...
            <div className="bg-[#0a0e14]/50 border border-slate-700/50 p-4 rounded-lg flex items-center justify-between group hover:border-amber-700/50 transition-colors">
              ...
              >
                <i className="fa-solid fa-download"></i> Tải Mã Nguồn V19
              </button>
            </div>
