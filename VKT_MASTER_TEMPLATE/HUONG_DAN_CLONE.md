...
> **Phiên bản:** V17.0 — "Đỉnh Của Đỉnh" (Director Advisor + Zero-Error Build)  
...
- ✅ **[MỚI V17] Cố Vấn Đạo Diễn AI** — Floating chatbot tư vấn kịch bản, kiểm tra chính sách, đề xuất hook/tiêu đề viral (nổi góc phải màn hình, có fallback offline)
- ✅ **[MỚI V17] Song Ngữ VI/EN** — Hệ thống translations.ts chuẩn hóa toàn bộ text giao diện
- ✅ **[MỚI V17] Prompts Tổng Quát Hóa** — Toàn bộ ngách cứng (Phật giáo, Hoa sen) đã được thay bằng biến số động `[NICHE_THEME]`, `[NICHE_VIBE]`, `[WATERMARK]`
- ✅ **[MỚI V17] VKT_CREATIVE_DIRECTOR_PROMPT.md** — Bộ não kịch bản chuẩn Markdown, có Lưới Lọc Chính Sách, Ma Trận Viral, Cơ Chế Tự Kiểm Duyệt Kép
...
### 🔒 QUY TẮC THƯƠNG HIỆU BẮT BUỘC: "MÀU ĐỒNG NHẤT — ICON KHÁC NHAU"

> ⚠️ **ĐÂY LÀ QUY TẮC KHÔNG BAO GIỜ ĐƯỢC PHÉP VI PHẠM**

Toàn bộ hệ sinh thái VKT chỉ dùng **MỘT bảng màu duy nhất: Amber/Gold**.

**Khi clone ngách mới, CHỈ ĐƯỢC ĐỔI icon trong `AIAssistant.tsx`:**

| Ngách | Icon (lucide-react) | Tooltip text |
|-------|--------------------|----|
| Generic Studio | `Film` | 🎬 Cố vấn Đạo diễn |
| Kids Cartoon | `Star` | 🌟 Cố vấn Kids Studio |
| Dharma/Tâm linh | `Flame` | 🕯️ Cố vấn Pháp thoại |
| Tái chế/Eco | `Leaf` | 🌿 Cố vấn Eco Studio |
| Horror/Kinh dị | `Moon` | 🌙 Cố vấn Horror Studio |
| Finance/Tài chính | `TrendingUp` | 📈 Cố vấn Finance Studio |
| Cooking/Ẩm thực | `ChefHat` | 👨‍🍳 Cố vấn Ẩm thực |

**Các màu GIỮ NGUYÊN 100% — không bao giờ đổi:**
```
Primary:    #f59e0b   ← amber-500
Dark:       #92400e   ← amber-900  
Background: #0a0e14   ← slate dark
Border:     rgba(245,158,11,0.2)
Online dot: #34d399   ← emerald-400
```
...
| 3 | Dùng backtick lồng trong prompts.ts | Build fail: Invalid Unicode/Unterminated | Dùng string concatenation thay vì template literal lồng nhau |
...
| 7 | Quên sửa translations.ts | Header hiển thị sai tên/subtitle | Sửa text trong `src/data/translations.ts` |
...
├── 📄 VKT_CREATIVE_DIRECTOR_PROMPT.md ← [MỚI V17] Bộ não kịch bản chuẩn
├── 📄 VKT_CODING_RULES.md         ← Quy tắc code toàn hệ thống
...
│   │   ├── ApiKeyModal.tsx        ← Giữ nguyên (Két Sắt API UI)
│   │   ├── AIAssistant.tsx        ← [MỚI V17] Cố Vấn Đạo Diễn AI (nổi góc phải)
│   │   └── Toast.tsx              ← Giữ nguyên
│   ├── 📁 data/
│   │   ├── constants.ts           ← ⚠️ ĐÂY LÀ FILE CHÍNH #1 (Bước 2)
│   │   ├── prompts.ts             ← ⚠️ ĐÂY LÀ FILE CHÍNH #2 (Bước 2)
│   │   └── translations.ts        ← [MỚI V17] ⚠️ Đổi text giao diện VI/EN
│   ├── 📁 services/
...
| Dharma Studio | *(xem VKT_ECOSYSTEM_HUB)* | `e:\HMKT\VKT_DHARMA_STUDIO - P` |
| *(Thêm ngách mới vào đây)* | — | — |

---

**© VKT — 055.979.3678**  
*Template V17.0 "Đỉnh Của Đỉnh" — Director Advisor + Zero-Error Build — Dùng chung cho mọi ngách nội dung*
