...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Tự động khôi phục kịch bản phiên cũ từ localStorage khi F5
    // ⚠️ KHI CLONE: Prefix 'kids_' đã được set đúng cho ngách này
    const saved = localStorage.getItem('kids_autosave_script');
    if (saved) {
      try {
  ...
  const handleUseStrategy = (title: string) => {
    ...
    setScriptSegments([]);
    setScriptData(null);
    localStorage.removeItem('kids_autosave_script');
    setActiveTab('script');
  };
