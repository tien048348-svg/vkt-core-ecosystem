...
// Clean up any old injections
content = content.replace(/👑 2\. CHỈ THỊ THANH ÂM THIẾT QUÂN LUẬT[\s\S]*?BGM luôn giữ âm lượng ổn định ở mức tĩnh -20dB, không trồi sụt\.\s*/g, '');
content = content.replace(/# 2\. CHỈ THỊ THANH ÂM THIẾT QUÂN LUẬT[\s\S]*?BGM luôn giữ âm lượng ổn định ở mức tĩnh -20dB, không trồi sụt\.\s*/g, '');

content = content.replace(/Giao thức Liền mạch Âm thanh \(Audio Continuity\):[\s\S]*?-20dB\./g, '');
content = content.replace(/Lời thoại Nội lực \(voice_text\):[\s\S]*?<40 từ\./g, '');
content = content.replace(/Bản đồ Thanh âm Thích ứng \(Adaptive Blueprint\):[\s\S]*?- Vị trí \(State\)/g, '');
content = content.replace(/Nguyên tắc Độc tôn \(100% Single Voice\):[\s\S]*?tiếng nói duy nhất\./g, '');
content = content.replace(/??? 2\. CHỈ THỊ THANH ÂM THIẾT QUÂN LUẬT \(HARDCODED AUDIO\)[\s\S]*?quy tắc thép:/g, '');


// Find the injection point which is exactly after "Cơ chế: Phân tích -> Trích xuất -> Hiệu chỉnh -> Tái xuất JSON."
const injectionPointStr = "Cơ chế: Phân tích -> Trích xuất -> Hiệu chỉnh -> Tái xuất JSON.";
if (content.includes(injectionPointStr)) {
  content = content.replace(injectionPointStr, injectionPointStr + "\\n" + audioProtocol);
} else {
  // Try the older version
  const oldInjectionPoint = "Yêu cầu thực hiện hiệu chỉnh duy nhất phần Thanh âm cho [Cảnh 1 đến Cảnh N]. Hệ thống phải vận hành theo cơ chế 'Phong tỏa Tham số - Tái cấu trúc Hồn'.";
  content = content.replace(oldInjectionPoint, oldInjectionPoint + "\\n" + audioProtocol);
}

fs.writeFileSync(path, content, 'utf8');
console.log('Fixed audio prompt in VKT_KIDS');
